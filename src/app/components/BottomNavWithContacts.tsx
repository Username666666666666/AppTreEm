import { Home, ListTodo, PhoneCall, Gift, User } from 'lucide-react';

interface BottomNavWithContactsProps {
  activeTab: 'home' | 'tasks' | 'contacts' | 'rewards' | 'profile'| 'videos';
  onTabChange: (tab: 'home' | 'tasks' | 'contacts' | 'rewards' | 'profile'| 'videos') => void;
}

export function BottomNavWithContacts({ activeTab, onTabChange }: BottomNavWithContactsProps) {
  const tabs = [
    { id: 'home' as const, icon: Home, label: 'Trang chủ' },
    { id: 'tasks' as const, icon: ListTodo, label: 'Nhiệm vụ' },
    { id: 'contacts' as const, icon: PhoneCall, label: 'Liên hệ' },
